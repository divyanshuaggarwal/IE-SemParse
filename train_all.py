...
def make_combined_dataset(dataset_name, model_checkpoint):
    ...
    final_dataset['train'] = concatenate_datasets([dataset['train'] for dataset in datasets])
    final_dataset['val'] = concatenate_datasets([dataset['val'] for dataset in datasets])
    final_dataset['test'] = concatenate_datasets([dataset['test'] for dataset in datasets])
    ...
    for dataset_name in dataset_names:
        ...
        for model_checkpoint in list(seq2seq_models + encoder_models):
            ...
            if model_name in os.listdir(f"{base_path}/train_all/{dataset_name}/"):
                if len(os.listdir(f"{base_path}/train_all/{dataset_name}/{model_name}")) == len(INDIC):
                    continue
            ...
            if model_checkpoint in encoder_models:
                model = get_model(model_checkpoint,
                                  tokenizer, encoder_decoder=True)
            ...
            for lang in INDIC:
                ...
                if f"{lang}.json" in os.listdir(f"{base_path}/train_all/{dataset_name}/{model_name}/"):
                    print("Skipping.......")
                    continue
